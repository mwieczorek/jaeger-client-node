...
describe('test tchannel span bridge', function() {
  ...
  _lodash2.default.each(options, function(o) {
    ...
    it(o.description + ' spans propagate through tchannel and preserve parent span properties', function(
      done
    ) {
      ...
      function onServerListening(err, res, arg2, arg3) {
        ...
        var clientCallback = function clientCallback(err, res, headers, body) {
          ...
          serverSpanTags[_opentracing2.default.Tags.SPAN_KIND] =
            _opentracing2.default.Tags.SPAN_KIND_RPC_SERVER;
          ...
          clientSpanTags[_opentracing2.default.Tags.SPAN_KIND] =
          ...
          _chai.assert.equal(
            serverSpan.context().traceIdStr === originalSpan.context().traceIdStr,
            !!o.context
          );
          _chai.assert.equal(
            clientSpan.context().traceIdStr === originalSpan.context().traceIdStr,
            !!o.context
          );
  ...
  it('span logs an error that occurred while propagating through tchannel', function(done) {
    var mockSpan = tracer.startSpan('checkErr');
    var mockCallback = function mockCallback(err, res) {};
    var res = { ok: false, body: {} };
    var err = 'some error occurred while propagating';
    bridge._tchannelCallbackWrapper(mockSpan, mockCallback, err, res);

    _chai.assert.equal(mockSpan._logs.length, 1);
    _chai.assert.equal(mockSpan._logs[0].fields[0].key, 'event');
    _chai.assert.equal(mockSpan._logs[0].fields[0].value, 'error');
    _chai.assert.equal(mockSpan._logs[0].fields[1].key, 'message');
    _chai.assert.equal(mockSpan._logs[0].fields[1].value, err);
    done();
  }).timeout(BIG_TIMEOUT);
});
