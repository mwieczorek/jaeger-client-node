...
describe('Zipkin B3 Text Map Codec should', function() {
  ...
  beforeEach(function() {
    ...
    tracer = new _tracer2.default(
      ...
      new _const_sampler2.default(false),
      {
        metrics: metrics,
      }
    );
  ...
  it("should normalize header's case and return a context", function() {
    /**
    ...
    _chai.assert.isOk(context.traceIdStr);
    _chai.assert.isOk(context.spanIdStr);
    _chai.assert.isOk(context.parentIdStr);
    _chai.assert.isOk(context.isSampled());
    _chai.assert.isOk(context.isDebug());
    _chai.assert.isOk(context.debugId);
  });
  ...
  it('not set the sampled flag if sampling is denied', function() {
    var headers = {
      'x-b3-sampled': '0',
    };
  ...
  it('handle true value for the sampled header', function() {
    var headers = {
      'x-b3-sampled': 'true',
    };
  ...
  it('handle false value for the sampled header', function() {
    var headers = {
      'x-b3-sampled': 'false',
    };
